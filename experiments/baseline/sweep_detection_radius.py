"""
Parameter sweep: Detection radius vs success probability.

Evaluates the greedy baseline policy across a range of detection radii
to quantify how sensing range affects interception success.

Usage:
    python experiments/baseline/sweep_detection_radius.py [--n-episodes N] [--radii "5,8,10,12,15"]
"""
...
def sweep_detection_radius(
    ...
    """
    Run parameter sweep over detection radii.
    ...
    if verbose:
        print("=" * 60)
        print("PARAMETER SWEEP: DETECTION RADIUS")
        print("=" * 60)
        print(f"Radii:        {radii}")
...
def plot_sweep(df: pd.DataFrame, output_path: str, show: bool = False) -> None:
    ...
    # Add reference line for intercept radius
    ...
    ax.set_title('Greedy Baseline: Success Rate vs Detection Radius', fontsize=14)
    ax.set_ylim(0, 100)
    ax.set_xlim(df["detection_radius"].min() - 1, df["detection_radius"].max() + 1)
    ax.grid(True, alpha=0.3)
    ax.legend(loc='lower right')
    
    # Add detection-to-intercept ratio as secondary x-axis
    ax2 = ax.twiny()
    ax2.set_xlim(ax.get_xlim())
    ax2.set_xticks(df["detection_radius"])
    ratios = df["detection_radius"] / intercept_radius
...
def main():
    ...
    print("SWEEP RESULTS")
    print("=" * 60)
    print(f"{'Radius':>8} {'Det Rate':>10} {'Success':>10} {'SE':>8} {'Ep Len':>8}")
    print("-" * 60)
