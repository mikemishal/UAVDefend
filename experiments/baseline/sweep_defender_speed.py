"""
Parameter sweep: Defender speed vs success probability.

Evaluates the greedy baseline policy across a range of defender speeds
to quantify how speed advantage affects interception success.

Usage:
    python experiments/baseline/sweep_defender_speed.py [--n-episodes N] [--speeds "10,12,14,16,18,20"]
"""
...
def sweep_defender_speed(
    ...
    """
    Run parameter sweep over defender speeds.
    ...
    if verbose:
        print("=" * 60)
        print("PARAMETER SWEEP: DEFENDER SPEED")
        print("=" * 60)
...
def plot_sweep(df: pd.DataFrame, output_path: str, show: bool = False) -> None:
    ...
    ax.set_title('Greedy Baseline: Success Rate vs Defender Speed', fontsize=14)
    ax.set_ylim(0, 100)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parameter sweep: defender speed vs success rate"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f"Comma-separated list of defender speeds (default: {default_speeds})"
    )
    parser.add_argument(
