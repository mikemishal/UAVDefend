"""
Parameter sweep: Enemy speed vs success probability (Kalman baseline).

Evaluates the KalmanGreedyInterceptPolicy across a range of enemy speeds
with use_kalman_tracking=True.  Directly comparable to:
    - experiments/baseline/sweep_enemy_speed.py         (greedy, no Kalman)
    - experiments/rl/sweep_enemy_speed_rl.py            (PPO Direct RL)
    - experiments/rl_kalman/sweep_enemy_speed_rl_kalman.py (PPO RL-Kalman)

Usage:
    python experiments/baseline/sweep_enemy_speed_kalman_baseline.py [--n-episodes N] [--speeds "8,10,12,14,16"]
"""
...
def sweep_enemy_speed(
    ...
    """
    Run enemy-speed sweep with the Kalman greedy baseline.

    Args:
        speeds: Enemy speeds to evaluate.
    ...
    if verbose:
        print("=" * 65)
        print("PARAMETER SWEEP: ENEMY SPEED  [Kalman Baseline]")
        print("=" * 65)
    ...
    for v_e in speeds:
        ...
        def env_factory(speed=v_e):
            config = EnvConfig(v_e=speed, use_kalman_tracking=True)
        ...
        results.append({
            ...
            "speed_ratio": defender_speed / v_e,
            "n_episodes": n,
            "n_success": int(p * n),
            "success_rate": p,
            "success_se": se,
...
def plot_sweep(df: pd.DataFrame, output_path: str, show: bool = False) -> None:
    """Plot success rate vs enemy speed with error bars and tracking error panel."""
    ...
    ax.set_title("Kalman Baseline: Success Rate vs Enemy Speed", fontsize=13)
    ax.set_ylim(0, 100)
    ...
    ax2.set_xticklabels([f"{defender_speed / v_e:.2f}" for v_e in df["enemy_speed"]])
    ax2.set_xlabel("Speed Ratio (v_d / v_e)", fontsize=10)

    # --- Right: tracking error vs enemy speed ---
    ax3 = axes[1]
    ...
    if len(tracked) > 0:
        ...
        ax3.set_xlabel("Enemy Speed (v_e)", fontsize=12)
        ax3.set_ylabel("Mean Tracking Error (m)", fontsize=12)
        ax3.set_title("Kalman Baseline: Tracking Error vs Enemy Speed", fontsize=13)
        ax3.set_ylim(bottom=0)
        ax3.set_xlim(df["enemy_speed"].min() - 1, df["enemy_speed"].max() + 1)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parameter sweep: enemy speed vs success rate (Kalman baseline)"
    )
    ...
    csv_path = os.path.join(output_dir, get_sweep_csv_filename("enemy_speed", POLICY_KEY))
    plot_path = os.path.join(output_dir, get_sweep_plot_filename("enemy_speed", POLICY_KEY))
