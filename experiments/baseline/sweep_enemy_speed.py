"""
Parameter sweep: Enemy speed vs success probability.

Evaluates the greedy baseline policy across a range of enemy speeds
to quantify how performance degrades as the attacker gets faster.

Usage:
    python experiments/baseline/sweep_enemy_speed.py [--n-episodes N] [--speeds "8,10,12,14,16"]
"""
...
def sweep_enemy_speed(
    ...
    """
    Run parameter sweep over enemy speeds.
    ...
    if verbose:
        print("=" * 60)
        print("PARAMETER SWEEP: ENEMY SPEED")
        print("=" * 60)
...
def plot_sweep(df: pd.DataFrame, output_path: str, show: bool = False) -> None:
    ...
    ax.set_title('Greedy Baseline: Success Rate vs Enemy Speed', fontsize=14)
    ax.set_ylim(0, 100)
    ...
    ax2.set_xticklabels([f'{r:.2f}' for r in df["speed_ratio"]])
...
def main():
    parser = argparse.ArgumentParser(
        description="Parameter sweep: enemy speed vs success rate"
    )
    parser.add_argument(
