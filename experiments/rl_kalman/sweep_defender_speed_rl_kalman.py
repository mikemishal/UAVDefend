"""
Parameter sweep: Defender speed vs success probability (RL-Kalman policy).

=============================================================================
EXPERIMENT TRACK: RL WITH KALMAN-ESTIMATED ENEMY STATE
=============================================================================

Evaluates the RL-Kalman PPO policy across a range of defender speeds.
Mirrors experiments/rl/sweep_defender_speed_rl.py for comparison.

Usage:
    python experiments/rl_kalman/sweep_defender_speed_rl_kalman.py \\
...
def sweep_defender_speed(
    ...
    """Run parameter sweep over defender speeds using RL-Kalman policy."""
    if verbose:
        print("=" * 60)
        print("PARAMETER SWEEP: DEFENDER SPEED (RL-KALMAN)")
        print("=" * 60)
...
def plot_sweep(df: pd.DataFrame, output_path: str, show: bool = False) -> None:
    ...
    ax.errorbar(
        ...
        linewidth=2,
        markersize=8,
        color='#E74C3C',  # Red for RL-Kalman
        ecolor='#34495E',
        label='RL-Kalman (PPO)'
    )
    
    enemy_speed = df["enemy_speed"].iloc[0]
    ax.axvline(x=enemy_speed, color='gray', linestyle='--', alpha=0.7,
               label=f'Enemy speed (v_e = {enemy_speed})')
    
    ax.set_xlabel('Defender Speed (v_d)', fontsize=12)
    ax.set_ylabel('Success Rate (%)', fontsize=12)
    ax.set_title('RL-Kalman Policy: Success Rate vs Defender Speed', fontsize=14)
    ax.set_ylim(0, 100)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parameter sweep: defender speed (RL-Kalman)"
    )
